...
<main class="p-6"><div class="max-w-xl">
	<p class="text-sm text-gray-500 mb-4">Enter actual sq ft received. Leave blank to skip a line.</p>
		...
		<div class="card mb-4">
			<table class="w-full text-sm">
				...
				<tr class="border-b border-gray-50">
					...
					<td class="px-4 py-2 text-right font-mono text-gray-500">{Math.round(line.sqft_ordered).toLocaleString()}</td>
					<td class="px-4 py-2">
						<input type="number" name="sqft_{line.id}" step="1" min="0"
							placeholder={Math.round(line.sqft_ordered)}
							class="form-input text-sm text-right font-mono w-full" />
					</td>
				</tr>
				{:else}
				<tr><td colspan="3" class="px-4 py-4 text-gray-400 text-sm text-center">All lines already received.</td></tr>
				{/each}
				</tbody>
			</table>
		</div>
