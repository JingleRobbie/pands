<script>
	...
	function fmtDateFull(d) { return new Date(d).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric', timeZone: 'UTC' }); }
</script>
...
<div class="card">
	<div class="card-header"><span class="font-semibold text-sm text-gray-700">Today — {fmtDateFull(data.today)}</span></div>
	...
	<table class="w-full text-sm">
		...
		<tr class="border-b border-gray-50 hover:bg-gray-50">
			...
			<td class="px-4 py-2 text-right font-mono">{Math.round(run.sqft_scheduled).toLocaleString()}</td>
			<td class="px-4 py-2 text-right"><a href="/production/{run.id}/confirm" class="btn-primary btn-sm">Confirm</a></td>
		</tr>
		{/each}
		</tbody>
	</table>
...
<div class="card">
	<div class="card-header"><span class="font-semibold text-sm text-gray-700">Upcoming</span></div>
	...
	<table class="w-full text-sm">
		...
		<tr class="border-b border-gray-50 hover:bg-gray-50">
			...
			<td class="px-4 py-2 font-mono text-xs">{run.run_number}</td>
			<td class="px-4 py-2"><a href="/production/{run.id}/confirm" class="hover:underline">{run.job_name}</a></td>
			<td class="px-4 py-2">{run.display_label}</td>
			<td class="px-4 py-2 text-right font-mono">{Math.round(run.sqft_scheduled).toLocaleString()}</td>
		</tr>
		{/each}
