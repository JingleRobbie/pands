<script>
	...
	let selectedLineObj = $derived(availableLines.find(l => String(l.id) === String(selectedLine)));
		...
		<div class="card mb-4">
			...
			<div class="card-body space-y-4">
				...
				<div>
					...
					<select class="form-select" bind:value={selectedSo} onchange={() => { selectedLine = ''; sqft = ''; }}>
						<option value="">— select SO —</option>
						{#each data.sos as so}
							<option value={String(so.id)}>SO {so.so_number} — {so.job_name}</option>
						{/each}
					</select>
				</div>
				...
				<div>
					...
					<select name="so_line_id" class="form-select" value={selectedLine} onchange={onLineChange} required>
						<option value="">— select line —</option>
						{#each availableLines as line}
							<option value={String(line.id)}>{line.display_label} — {Math.round(line.sqftUnscheduled).toLocaleString()} sqft unscheduled</option>
						{/each}
					</select>
				</div>
				{:else if selectedSo}
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="form-label">Run Date <span class="text-gray-400 font-normal text-xs">(blank = unscheduled)</span></label>
						<input type="date" name="run_date" class="form-input" bind:value={runDate} />
					</div>
					<div>
						<label class="form-label">Sq Ft to Schedule</label>
						<input type="number" name="sqft_scheduled" step="1" min="1"
							class="form-input font-mono" bind:value={sqft} required />
						{#if selectedLineObj}
						<p class="text-xs text-gray-400 mt-1">Unscheduled: {Math.round(selectedLineObj.sqftUnscheduled).toLocaleString()} sqft</p>
						{/if}
					</div>
				</div>
				{/if}
