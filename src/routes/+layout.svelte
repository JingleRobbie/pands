	...
	<aside class="w-56 min-h-screen flex-shrink-0 flex flex-col" style="background-color:#1e2433;">
		...
		<nav class="flex-1 py-4 space-y-0.5 px-2">
			{#each [
				...
				{ href: '/so',           label: 'Sales Orders',     icon: 'doc' },
				...
				<a href={nav.href}
				   class="flex items-center gap-2.5 px-3 py-2 rounded-md text-sm font-medium transition-colors
				          {$page.url.pathname.startsWith(nav.href) ? 'bg-blue-600 text-white' : 'text-white/70 hover:bg-white/10 hover:text-white'}">
					{#if nav.icon === 'grid'}
						<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 10h18M3 14h18M10 3v18M14 3v18"/></svg>
					...
					{:else}
						<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
					{/if}
					{nav.label}
				</a>
			{/each}
