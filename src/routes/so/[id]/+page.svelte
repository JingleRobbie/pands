<script>
	...
	function fmtDate(d) { if (!d) return '?'; return new Date(d).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', timeZone: 'UTC' }); }
</script>
...
<main class="p-6"><div class="max-w-3xl space-y-3">
	<div class="card">
		<div class="card-body grid grid-cols-3 gap-4 text-sm">
			<div><span class="text-gray-500">Ship Date</span><p class="font-medium mt-0.5">{new Date(so.ship_date).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric', timeZone: 'UTC' })}</p></div>
	...
	<div class="card">
		<div class="card-header">
			<span class="font-semibold text-sm text-gray-800">{item.line.display_label}</span>
			<div class="flex items-center gap-4 text-xs text-gray-500">
				<span>Ordered: <strong class="text-gray-800 font-mono">{Math.round(item.line.sqft_ordered).toLocaleString()}</strong> sqft</span>
				<span>Produced: <strong class="text-gray-800 font-mono">{Math.round(item.line.sqft_produced).toLocaleString()}</strong> sqft</span>
				{#if item.sqftUnscheduled > 0}
					<span class="text-amber-700">Unscheduled: <strong class="font-mono">{Math.round(item.sqftUnscheduled).toLocaleString()}</strong> sqft</span>
				{:else}
		...
		<table class="w-full text-xs">
			...
			<tr class="border-b border-gray-50">
				...
				<td class="px-4 py-1.5 text-right font-mono">{Math.round(run.sqft_scheduled).toLocaleString()}</td>
				<td class="px-4 py-1.5">
					{#if run.status === 'SCHEDULED'}<span class="badge-blue">Scheduled</span>
