...
export const actions = {
	default: async ({ request, locals }) => {
		...
		if (!soNumber || !jobName || !shipDate)
			return fail(400, { error: 'SO number, job name, and ship date are required.' });

		const [[existing]] = await db.query('SELECT id FROM sales_orders WHERE so_number = ?', [
			soNumber,
		]);
