<script>
	...
	function addLine() { lines = [...lines, { sku_id: '', sqft: '' }]; }
	function removeLine(i) { if (lines.length > 1) lines = lines.filter((_, idx) => idx !== i); }
</script>
...
<main class="p-6"><div class="max-w-2xl">
	...
	<form method="POST" use:enhance>
		<div class="card mb-4">
			<div class="card-header"><span class="font-semibold text-sm text-gray-700">Order Details</span></div>
			<div class="card-body grid grid-cols-3 gap-4">
				<div>
					<label class="form-label">SO Number</label>
					<input type="text" name="so_number" class="form-input" required placeholder="e.g. 12345" />
				</div>
				<div>
					<label class="form-label">Job Name</label>
					<input type="text" name="job_name" class="form-input" required placeholder="e.g. Job 12" />
				</div>
				<div>
			...
			<div class="card-body space-y-3">
				...
				<div class="flex items-end gap-3">
					<div class="flex-1">
						...
						<select name="sku_id" class="form-select text-sm" bind:value={line.sku_id} required>
							<option value="">— select —</option>
							{#each data.skus as sku}<option value={sku.id}>{sku.display_label}</option>{/each}
						</select>
					</div>
					<div class="w-36">
						<label class="form-label text-xs">Sq Ft</label>
						<input type="number" name="sqft_ordered" step="1" min="1"
							class="form-input text-sm text-right font-mono" bind:value={line.sqft}
							required placeholder="0" />
					</div>
					{#if lines.length > 1}
					<button type="button" onclick={() => removeLine(i)}
						class="text-gray-400 hover:text-red-500 text-xl pb-2">&times;</button>
					{/if}
				</div>
				{/each}
			</div>
