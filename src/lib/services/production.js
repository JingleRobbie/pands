...
export async function scheduleRun(soLineId, runDate, sqftScheduled, userId) {
	...
	try {
		...
		const [[line]] = await conn.query(
			'SELECT * FROM sales_order_lines WHERE id = ? FOR UPDATE', [soLineId]
		);
		...
		const unscheduled = Number(line.sqft_ordered) - Number(line.sqft_produced) - Number(sqftInRuns);
		if (sqftScheduled <= 0) throw new Error('Scheduled sq ft must be greater than zero.');
		if (sqftScheduled > unscheduled) {
			throw new Error(`Cannot schedule ${sqftScheduled} sqft — only ${unscheduled.toFixed(0)} sqft unscheduled.`);
		}
		...
		await conn.query(`
			...
			VALUES (?, ?, ?, ?, ?, ?, ?)
		`, [runNumber, soLineId, line.sku_id, runDate || null, sqftScheduled, status, userId ?? null]);
...
export async function confirmRun(runId, sqftActual, userId) {
	...
	try {
		...
		const [[run]] = await conn.query(
			'SELECT * FROM production_runs WHERE id = ? FOR UPDATE', [runId]
		);
		...
		const [[sol]] = await conn.query('SELECT * FROM sales_order_lines WHERE id = ?', [run.so_line_id]);
		...
		if (incomplete === 0) {
			await conn.query('UPDATE sales_orders SET status = "COMPLETE" WHERE id = ?', [sol.so_id]);
		}
